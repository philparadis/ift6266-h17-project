...
if [[ -d "${target_dir}" ]]; then
	...
	echo "Okay, directory is valid! We can skip extracting and downloading the archive."
	skip_extract=1
	skip_download=1
    else
...
if [[ -f "${file}" ]]; then
	...
	echo "Okay, file '$file' already exists and checksum is valid, will skip downloading archive..."
	skip_download=1
    fi
fi
...
# Create test directory (even if appears to exist, for safety's sake, it's a small amount of data)
