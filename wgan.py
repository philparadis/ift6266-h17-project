...
def build_generator(input_var=None):
    ...
    layer = batch_norm(Deconv2DLayer(layer, 128, 5, stride=2, crop='same', output_size=8))
    layer = DropoutLayer(layer, p=0.5)
    layer = batch_norm(Deconv2DLayer(layer, 64, 5, stride=2, crop='same', output_size=16))
    layer = DropoutLayer(layer, p=0.5)
    layer = batch_norm(Deconv2DLayer(layer, 32, 5, stride=2, crop='same', output_size=32))
    layer = DropoutLayer(layer, p=0.5)
    layer = Deconv2DLayer(layer, 3, 5, stride=2, crop='same', output_size=64, nonlinearity=T.tanh)
...
def build_critic(input_var=None):
    ...
    # two convolutions
    layer = batch_norm(Conv2DLayer(layer, 64, 5, stride=2, pad='same', nonlinearity=lrelu))
    layer = batch_norm(Conv2DLayer(layer, 128, 5, stride=2, pad='same', nonlinearity=lrelu))
    layer = batch_norm(Conv2DLayer(layer, 256, 5, stride=2, pad='same', nonlinearity=lrelu))
    layer = batch_norm(Conv2DLayer(layer, 512, 5, stride=2, pad='same', nonlinearity=lrelu))
